...
def print_result(result):
    ...
    if result.get("error"):
        ...
        try:
            ...
            print(
                f"   Response: {json.dumps(data, indent=2, ensure_ascii=False)[:300]}"
            )
        except:
...
def main():
    ...
    # 14. Listar Preços
    r = test_with_result(
        "Stripe Prices List", requests.get, f"{BASE_URL}/stripe/prices", 200
    )
    print_result(r)
    results.append(r)
    ...
    # 16. Debug Hash Test
    r = test_with_result(
        "Debug Hash Test", requests.get, f"{BASE_URL}/debug/hash-test", 200
    )
    print_result(r)
    results.append(r)
    ...
    if failed > 0:
        ...
        for r in results:
            if not r["success"]:
                print(
                    f"   - {r['name']}: {r.get('error') or f'Status {r['status']} (expected {r.get('expected', '2xx')}'}"
                )
